function [tnList,ynList] = RK2(func,tspan,N,Y0,alpha)
%
...
for k = 1:N
    ...
    fnMidpoint = feval(func,tnList(k) + alpha*dt ,ynList(:,k) + alpha*dt * f);

    ynList(:,k+1) = ynList(:,k)+ dt*(1 - (1/2*alpha))*f + (dt/(2*alpha))*fnMidpoint;
